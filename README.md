...
- `GET /v1/runs/{run_id}/coverage.csv` — the same matrix as CSV (`Content-Disposition: attachment`): one row per framework × control label × target with `count`, `max_severity`, the contributing `categories`, the `unmapped` flag and a `basis` column (`category-level heuristic`) repeated on every row so the honesty label survives the export; cells starting with `=`, `+`, `-`, `@` are prefixed with `'` so a spreadsheet does not evaluate attacker-influenced category text as a formula. CSV is the export format; there is no PDF
