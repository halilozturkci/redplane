...
- `GET /v1/runs/{run_id}/traces` — the run's linked gateway traces from `run_manifest.json` `gateway_traces` (idea 3): `by_run_id` (the attack tool forwarded `X-URT-Run-Id` = `URT_RUN_ID` from its environment — exact) and `by_time_window` (untagged requests inside the run's `[started_at, completed_at]` — weaker), `trace_ids` (union), and `traces[]` summary rows each with `link: run_id | time_window`
...
Traces and runs:
- Every request writes `<audit.artifact_root>/YYYYMMDD/trace-<stamp>-<trace_id>.json` (headers redacted, bodies truncated). The response `metadata.audit_path` is that path **relative to the audit root**.
- Send `X-URT-Run-Id: <run_id>` and the trace records `run_id`. Engines receive `URT_RUN_ID` in their environment (`runtime.build_runtime_env`); `templates/promptfoo_eval.template.yaml` forwards it as a provider header. At the end of a run the orchestrator writes the linked `trace_ids` into `run_manifest.json` (`gateway_traces.by_run_id` exact, `by_time_window` for untagged requests inside the run window) and `urt serve-api` browses the tree under `/v1/traces` and `/ui/traces` — read-only, gateway not required.
