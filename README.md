...
uv run urt runs
uv run urt diff <RUN_A> <RUN_B> [--format text]            # new / resolved / persisting findings, scorecard + eval deltas
...
uv run urt serve-api --port 8000                           # JSON API + /ui (loopback only by default)
URT_API_KEY=... uv run urt serve-api --host 0.0.0.0        # non-loopback needs the shared key (bearer + /ui/login) or --unsafe-allow-unauthenticated
...
`data_exfiltration`, … see `CATEGORY_ALIASES` in `policy/mapping.py`); when that
happens the engine's spelling is kept in `metadata.category_reported`.
...
- `GET /v1/runs/{run_id}/coverage` — framework coverage matrix as JSON: per framework (OWASP LLM Top 10 2025, OWASP Agentic, MITRE ATLAS) the rows are labels × targets with `count`, `max_severity` and the contributing `categories`; an explicit `unmapped` row keeps categories with no mapping visible; `unmapped_categories` lists those no framework maps; `excluded_kinds` says which `finding_kind`s (coverage_gap, execution, eval) are left out; `note` carries the honesty label — mappings are **category-level heuristics** from `policy/mapping.py`, not per-test verdicts
- `GET /v1/runs/{run_id}/coverage.csv` — the same matrix as CSV (`Content-Disposition: attachment`): one row per framework × control label × target with `count`, `max_severity`, the contributing `categories`, the `unmapped` flag and a `basis` column (`category-level heuristic`) repeated on every row so the honesty label survives the export; cells starting with `=`, `+`, `-`, `@`, `|` (also after leading whitespace) are prefixed with `'` so a spreadsheet does not evaluate attacker-influenced category text as a formula. CSV is the export format; there is no PDF
- `GET /v1/matrix[?target=&name_prefix=&limit=12]` — **targets × runs** matrix for the lead: `runs[]` newest first (at most `limit`, 1..100, `truncated` says whether older runs were cut), `targets[]`, and `cells[target][run_id]` = that target's **per-target** scorecard inside the run (`critical`, `high`, …, `total_findings`, `success_count`, `total_attacks`, `asr_overall`, `status`) or `null` when the run did not include the target; runs without findings yet are not columns. `note` states that cells are per-target, not run totals
...
- `GET /v1/capabilities` — what a spec may contain, from `constants`: `targets`, `engines`, `evaluators`, `profiles`, `default_profile`, `evidence_levels`, `profile_defaults`, `policy_profiles`, `severity_levels`
...
- `GET /v1/traces` · `GET /v1/traces/{day}[?target_id=&status_code=&run_id=]` · `GET /v1/traces/{day}/{trace_id}` — **read-only** index over the gateway audit root (`URT_GATEWAY_TRACE_ROOT` / `--gateway-trace-root`, default `.urt_state/gateway`, i.e. the gateway's `audit.artifact_root`); works while the gateway is down. Days newest first with counts; per-day summary rows (`timestamp_utc`, `target_id`, `connector`, `route_source`, `status_code`, `latency_ms`, `run_id`, `day`, `audit_path`); the detail is the trace **as stored** (headers redacted, bodies truncated to `audit.max_body_bytes` by the gateway) with `audit_path` relative to the root. `day` must be `YYYYMMDD` and `trace_id` 32 hex chars (`400` otherwise); symlinks are skipped
- `GET /v1/runs/{run_id}/traces` — the run's linked gateway traces from `run_manifest.json` `gateway_traces` (idea 3): `by_run_id` (the attack tool forwarded `X-URT-Run-Id` = `URT_RUN_ID` from its environment — exact) and `by_time_window` (untagged requests inside the run's `[started_at, completed_at]` — weaker: a concurrent run or a manual request through the same gateway lands here too), `all_trace_ids` (union), `trace_ids` (**exact matches only** — the manifest's top-level key means "this run's traces", so window guesses never enter it), and `traces[]` summary rows each with `link: run_id | time_window`
- `GET /v1/gateway/sessions` — sessions of the **running** gateway, fetched server-side from `URT_GATEWAY_URL` + `/v1/sessions` (bearer `URT_GATEWAY_API_KEY` when set): `count`, `ttl_seconds`, `sessions[]` with `session_id`, `source` (`live`/`persisted`), `last_used_utc`, `idle_seconds`, `thread_id_present`, `live_client_present`. Identifiers and presence only, never content. `503` when no gateway is configured or reachable — sessions exist only in the gateway process
...
- `GET /ui/specs[?template=]` — spec builder: template list with smoke / real badges, a form generated from `constants` (profile with its budget/timeout defaults, evidence level, one target with `${VAR}`-only auth modes, engines with optional `params.command`, evaluators, policy profiles) and a YAML editor that round-trips with it (`POST /ui/specs/build`, `POST /ui/specs/load`). **Validate** (`POST /ui/specs/validate`) shows errors inline, whether each referenced variable is set (boolean), the profile defaults and the redacted resolved spec; **Probe targets** (`POST /ui/specs/probe`) runs the healthchecks. **Run (queue)** (`POST /ui/specs/run`) validates the same way — the `${VAR}`-only auth rule means the browser never posts an expanded credential, and a literal is refused with `400` before anything is queued — then hands the expanded spec to the process's run worker (the same path as `POST /v1/runs`) and redirects (`303`, or `HX-Redirect` for htmx) to `/ui/runs/{run_id}`, where stage events appear as the run executes. CSRF-protected and, with `URT_API_KEY`, session-gated like every other `/ui` mutation.
...
- `GET /ui/traces[?run_id=]` · `GET /ui/traces/{day}[?target_id=&status_code=&run_id=]` · `GET /ui/traces/{day}/{trace_id}` — gateway trace browser (§4.8): day → list → detail with redacted headers and truncated bodies **as stored**, filter by target / status / run id; `?run_id=` shows the traces `run_manifest.json` links to that run with the join kind (`run_id` exact vs `time_window`), and the run page links there ("Gateway traces (N)"). The days page also lists the running gateway's sessions (ids, last use, Foundry `thread_id` present / Copilot live client present — no content) when `URT_GATEWAY_URL` is set, and says so when it is not.
...
accepted) is passed; reach a remote machine through an SSH tunnel instead. The UI mutates two things only:
waivers (create and revoke, never delete) and the run queue (`/ui/specs` Run button; findings, severities and
bundles are never edited). Those form posts are CSRF-protected: a random
...
- `gateway.api_key` — shared secret for `/v1/chat/completions` and `/v1/sessions` (`Authorization: Bearer …`, `X-API-Key`, or `X-URT-API-Key`). `/healthz` stays open.
...
- `GET /v1/sessions` — live and persisted session ids with `last_used_utc`, `idle_seconds`, `thread_id_present`, `live_client_present`; never the thread id value or any content.

Traces and runs:
- Every request writes `<audit.artifact_root>/YYYYMMDD/trace-<stamp>-<trace_id>.json` (headers redacted — `authorization`, `*api-key*`, `*token*`, `*secret*`, `*password*`, `cookie` / `set-cookie` and `audit.redact_headers` — bodies truncated). The response `metadata.audit_path` is that path **relative to the audit root**.
- Send `X-URT-Run-Id: <run_id>` and the trace records `run_id`. Engines receive `URT_RUN_ID` in their environment (`runtime.build_runtime_env`); `templates/promptfoo_eval.template.yaml` forwards it as a provider header. At the end of a run the orchestrator writes `trace_ids` (exact `X-URT-Run-Id` matches only) into `run_manifest.json`, plus `gateway_traces` with `by_run_id`, `by_time_window` (untagged requests inside the run window, a weaker join kept separate) and `all_trace_ids` and `urt serve-api` browses the tree under `/v1/traces` and `/ui/traces` — read-only, gateway not required.
