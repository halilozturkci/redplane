...
- `POST /v1/specs/validate` — body `{"yaml": "..."}` or `{"spec": {...}}`. **Validate-only**: returns `200` with `ok`, `errors[]` (`RunSpec` validation messages, YAML parse errors, and the builder rule that every `targets[*].auth` leaf must be a `${VAR}` reference — literal credentials are rejected), `resolved_spec` (the same key-redacted, value-scrubbed view `urt validate` prints; `null` when invalid), `profile_defaults` for the chosen `run_profile`, `env_vars[]` (`{name, set}` for every `${VAR}` in the payload — a boolean, **never the value**), `spec_kind` / `kind`. `400` for a malformed body, `413` above 256 KiB
...
- `GET /ui/diff?a=&b=` — pick two runs: side-by-side scorecard with deltas, ASR-by-category deltas, new / resolved / persisting finding groups (linked into each run's explorer), evaluation metric deltas. Runs list rows link here ("compare…").
